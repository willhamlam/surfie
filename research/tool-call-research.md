...
Surf Copilot 的工具调用系统允许 AI agent 在对话过程中调用外部工具，获取结果后继续生成回复。整个系统是**可插拔的接入式架构**，目前接入了两个工具：

- **tavily_search** — 网页搜索（需要 Tavily API key，条件注册）
- **read_page** — 提取当前页面可读内容（始终可用，通过 content script + `@mozilla/readability` 实现）
...
│                    tools/read-page.ts                 │
│                    content/readability.ts (content script) │
├──────────────────────────────────────────────────────┤
...
**Tavily Search** (`src/lib/tools/tavily-search.ts`) — 有参数的工具：
...
**Read Page** (`src/lib/tools/read-page.ts`) — 无参数的工具：

```ts
export function getReadPageTool(): Tool {
  return {
    name: 'read_page',
    description: 'Extract the readable content of the current web page...',
    parameters: Type.Object({}),   // 无参数，始终读取当前活跃 tab
  }
}
```
...
### 3.2 工具注册 (`src/composables/useChat.ts:122-125`)

工具在每次流式循环开始时按条件组装为数组：

```ts
const tools = [
  ...(settings.tavilyApiKey ? [getTavilyTool()] : []),  // 有 API key 才注册
  getReadPageTool(),                                      // 始终注册（不需要外部 API key）
]
```

两种注册模式：
- **条件注册**（tavily_search）：没有 API key → 不加入数组 → 模型不知道有此工具
- **始终注册**（read_page）：不依赖外部服务密钥，直接加入

`buildContext` 中工具数组为空时不传 `tools` 字段，模型就不会尝试调用任何工具。
...
### 9.2 Tavily Search 实现 (`src/lib/tools/tavily-search.ts`)
...
### 9.3 Read Page 实现 (`src/lib/tools/read-page.ts` + `src/content/readability.ts`)

与 Tavily Search 不同，Read Page 不调用外部 API，而是通过 Chrome 扩展的 content script 机制提取当前页面内容。

**架构：Side Panel ↔ Content Script 消息传递**

```
Side Panel (useChat.ts)                Content Script (readability.ts)
        │                                        │
        ├─ chrome.tabs.query()                   │
        │   → 获取当前 tab ID                     │
        │                                        │
        ...
        │                                        │   ├─ 格式化 title/author/excerpt/textContent
        │                                        │   ├─ 截断到 50,000 字符
        │                                        │   └─ 加数据边界标记
...
1. **DOM 克隆**：`document.cloneNode(true)` — Readability 的 `parse()` 会修改传入的 DOM，必须 clone 避免破坏原始页面
2. **内容截断**：限制 50,000 字符，超出部分标记 `[content truncated]`，防止 token 过多
3. **数据边界标记**：返回内容用 `--- BEGIN/END PAGE CONTENT ---` 包裹，明确标注这是提取的网页文本而非指令，防止页面内容被模型误判为 prompt injection
4. **URL 守卫**：`chrome://` 和 `chrome-extension://` 页面无法注入 content script，提前抛出友好错误
5. **AbortSignal 支持**：用户取消操作时中断等待

**Content Script 注册**（`manifest.json`）：

```json
"content_scripts": [{
  "matches": ["<all_urls>"],
  "js": ["src/content/readability.ts"]
}]
```

`vite-plugin-web-extension` 自动识别并单独打包 content script（~35 kB，含 Readability 库）。

---

## 10. UI 渲染 (`src/sidepanel/MessageBubble.vue`)

工具调用在助手消息气泡内渲染，位于文本内容之后（底部）：
...
│                                 │
│ ┌─────────────────────────────┐ │
│ │ 🔍 Searching  香港天气  done│ │  ← 工具调用卡片（tavily_search）
│ │   [展开查看结果]            │ │
│ └─────────────────────────────┘ │
│ ┌─────────────────────────────┐ │
│ │ 📄 Reading page        done │ │  ← 工具调用卡片（read_page）
│ │   [展开查看结果]            │ │
│ └─────────────────────────────┘ │
└─────────────────────────────────┘
```

渲染顺序：Thinking → Text content → Streaming cursor → Tool calls

工具图标按类型区分：
- `read_page` → `FileText` 图标，显示 "Reading page"
- `tavily_search` → `Search` 图标，显示 "Searching" + 查询词
- 其他工具 → `Search` 图标，显示工具原始名称

工具详情折叠状态使用 `Set<string>` 按 `tc.id` 独立追踪，每个工具调用可单独展开/收起。
...
用户输入 "今天香港天气怎么样？"
    ...
    runStreamLoop() ─── 第 1 轮 ───────────────────────────────
         ...
         │
         ├─ buildContext(piMessages, systemPrompt, [tavilyTool, readPageTool])
...
用户输入 "这个页面讲了什么？"
         │
         ▼
    sendMessage() → runStreamLoop() ─── 第 1 轮 ──────────────
         │
         ├─ 模型决定调用 read_page（无参数）
         │   └─ toolcall_end → { name: 'read_page', arguments: {} }
         │
         ├─ executeToolCall() → executeReadPage(signal)
         │   │
         │   ├─ chrome.tabs.query() → 获取当前 tab ID 和 URL
         │   ├─ URL 守卫：chrome:// 页面 → 抛出错误
         │   └─ chrome.tabs.sendMessage(tabId, { type: 'READ_PAGE' })
         │       │
         │       ▼  (消息传递到 content script)
         │   Content Script (src/content/readability.ts)
         │       ├─ document.cloneNode(true)
         │       ├─ new Readability(clone).parse()
         │       ├─ 格式化：Title / Author / Excerpt / textContent
         │       ├─ 截断到 50,000 字符
         │       ├─ 加 "--- BEGIN/END PAGE CONTENT ---" 边界标记
         │       └─ sendResponse({ content: '...' })
         │       │
         │       ▼  (回到 side panel)
         │   resolve(response.content)
